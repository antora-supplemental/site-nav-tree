...
/**
 * Antora `getNavigation` often returns anonymous tree(s) `{ items: [...] }`
 * with no content/url. Wrapping those under a component root creates empty
 * depth-1 folders (Tools multi-module nav) or a single empty caret.
 * Flatten every anonymous wrapper by concatenating its items in place; keep
 * titled trees (Platforms: DevCentr / devcentr.org) as siblings.
 */
function flattenNavTrees (trees) {
  ...
  for (const t of trees) {
    if (!t) continue
    if (!t.content && !t.url && Array.isArray(t.items)) {
      out.push(...t.items)
    } else {
      out.push(t)
    }
...
/**
 * Overview/Home as a linked *parent* that wraps sibling articles is nonsense in
 * the forest (Platforms > devcentr.org > Overview > articles). Recursively
 * promote those children so Overview is a peer leaf that loads the index.
 */
function unwrapOverviewParents (items) {
  if (!Array.isArray(items) || !items.length) return items
  const out = []
  for (const item of items) {
    if (!item) continue
    const nested =
      Array.isArray(item.items) && item.items.length
        ? unwrapOverviewParents(item.items)
        : undefined
    if (isHomeNavItem(item) && nested && nested.length) {
      out.push({
        content: 'Overview',
        url: item.url,
    ...
    if (nested) out.push({ ...item, items: nested })
    else out.push(item)
  }
  return out
}
...
/**
 * Section landings with children: promote the parent's own article URL to an
 * Overview child and leave the parent as an unlinked section label.
 *
 * Linked parent + Overview child sharing one URL makes Default UI / SoftNav
 * treat the parent as current (first is-current-page / samePath), so Overview
 * appears to "disappear" into Email/Software stack when clicked. Unlinking the
 * parent keeps Overview as the only navigable leaf for that landing URL.
 *
 * Skips items already labeled Overview/Home (handled by unwrapOverviewParents).
 * If an Overview/Home child already owns the same URL, only strip the parent URL.
 */
function promoteLinkedSectionLandings (items) {
  if (!Array.isArray(items) || !items.length) return items
  return items.map((item) => {
    if (!item) return item
    const nested =
      Array.isArray(item.items) && item.items.length
        ? promoteLinkedSectionLandings(item.items)
        : undefined
    const withNested = nested ? { ...item, items: nested } : item
    if (!withNested.url || !Array.isArray(withNested.items) || !withNested.items.length) {
      return withNested
    }
    if (isHomeNavItem(withNested)) return withNested

    const parentUrl = normalizeUrl(withNested.url)
    const kids = withNested.items.slice()
    const existing = kids.find(
      (c) =>
        c &&
        isHomeNavItem(c) &&
        c.url &&
        normalizeUrl(c.url) === parentUrl
    )
    if (existing) {
      const { url, urlType, ...rest } = withNested
      return { ...rest, items: kids }
    }

    const overviewLeaf = {
      content: 'Overview',
      url: withNested.url,
      urlType: withNested.urlType || 'internal',
    }
    const { url, urlType, ...rest } = withNested
    return { ...rest, items: [overviewLeaf].concat(kids) }
  })
}
...
function buildSiteNavigation (contentCatalog, getNavigation, currentComponent, currentVersion, config = {}) {
  ...
  for (const component of components) {
    ...
    items = unwrapOverviewParents(items)
    items = promoteLinkedSectionLandings(items)
