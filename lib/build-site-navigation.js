 ...
 * - Leaf matching component URL/title → keep as **Home** (do not drop).
 * - Linked parent matching start URL → promote children, prepend Home.
 * Avoids Business Bootstrap > Business Bootstrap while retaining a Home child
...
function unwrapStartPageDuplicate (items, componentUrl, componentTitle) {
  ...
  const homeUrl = first.url || componentUrl
  const homeLeaf = {
    content: 'Home',
    url: homeUrl,
  ...
  // Leaf start-page link: rename to Home (root already links to the start page).
  if (!first.items || !first.items.length) {
    return [homeLeaf].concat(items.slice(1))
  }

  // Linked parent that is also the start page: promote its children, keep Home.
  if (urlMatch) return [homeLeaf].concat(first.items, items.slice(1))
  return items
}
...
/**
 * Point the component root at its Home child URL when present so the forest
 * "button" / breadcrumb target matches the Home landing (same URL today, but
 * keeps roots honest if Home ever diverges).
 */
