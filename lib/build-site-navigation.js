...
/**
 * Drop a leading start-page link that duplicates the component root title/URL
 * (e.g. `* xref:index.adoc[Business Bootstrap]` under a site-nav-tree root).
 */
function unwrapStartPageDuplicate (items, componentUrl, componentTitle) {
  ...
  // Leaf start-page link: drop it (root already links to the start page).
  if (!first.items || !first.items.length) return items.slice(1)

  // Linked parent that is also the start page: promote its children.
  if (urlMatch) return first.items.concat(items.slice(1))
  return items
}
